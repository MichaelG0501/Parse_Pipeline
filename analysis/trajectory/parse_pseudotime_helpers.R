####################
# parse_pseudotime_helpers.R
#
# Description:
#   Shared Monocle3/Seurat helper functions for sample-level Parse pseudotime,
#   linear projection, and distance-map workflows.
#
# Inputs:
#   parse_outs/Auto_parse_merged.rds
#   Optional cache: parse_outs/pseudotime_samples/Auto_parse_sample_pseudotime*.rds/csv
#
# Outputs:
#   Helper functions only; no output is written until called by workflow scripts.
#
# Methodology:
...
analysis_samples <- strsplit(
  Sys.getenv("AUTO_PARSE_PSEUDOTIME_SAMPLES", unset = paste(parse_samples, collapse = ",")),
  ",",
  fixed = TRUE
)[[1]]
...
root_sample <- Sys.getenv("AUTO_PARSE_ROOT_SAMPLE", unset = parse_root_sample)
min_cells_per_sample <- as.integer(Sys.getenv("AUTO_PARSE_MIN_CELLS_PER_SAMPLE", unset = as.character(parse_thresholds$pseudotime_min_cells_per_sample)))
sample_cols_all <- parse_sample_colours[names(parse_sample_colours) %in% sample_order_all]
