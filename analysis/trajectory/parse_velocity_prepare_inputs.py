#!/usr/bin/env python3
"""Prepare Parse RNA velocity inputs.

Description:
    Extracts non-PDO barcodes and metadata, writes chromosome-prefixed GTF
    references, downloads RepeatMasker hg38 annotations if missing, and prepares
    BAM symlinks for velocyto.

Inputs:
    parse_outs/Auto_parse_all_meta.rds-derived Trailmaker metadata in data/trailmaker/
    data/trailmaker/output_NACT1090_[A|B]_*/process/barcode_headAligned_anno.bam
    /rds/general/project/tumourheterogeneity1/live/ITH_sc/refdata-gex-GRCh38-2024-A/genes/genes.gtf.gz
    https://hgdownload.soe.ucsc.edu/goldenPath/hg38/database/rmsk.txt.gz if RepeatMasker cache is absent

Outputs:
    parse_outs/Auto_velocity/barcodes/*_nonPDO_barcodes.tsv
    parse_outs/Auto_velocity/tables/*_cell_metadata.csv
    parse_outs/Auto_velocity/ref/genes.GRCh38-1-1-3c.gtf
    parse_outs/Auto_velocity/ref/repeatmasker.GRCh38-1-1-3c.gtf
...
from parse_pipeline_config import PARSE_SAMPLES, PROJECT_ROOT, VELOCITY_GTF, VELOCITY_OUT  # noqa: E402
