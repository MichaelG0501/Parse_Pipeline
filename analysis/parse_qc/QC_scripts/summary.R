...
script_path <- if (length(file_arg) > 0) {
  ...
  normalizePath("/rds/general/ephemeral/project/spatialtranscriptomics/ephemeral/novogene/Auto_parse_qc_pipeline/analysis/summary/Auto_parse_summary.R")
}
...
summary_df <- read.csv(file.path("summary", "filtering_summary.csv"), check.names = FALSE)
...
thresholds <- read.csv(file.path("summary", "thresholds.csv"))
