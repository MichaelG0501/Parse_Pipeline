...
script_path <- if (length(file_arg) > 0) {
  ...
  normalizePath("/rds/general/ephemeral/project/spatialtranscriptomics/ephemeral/novogene/Auto_parse_qc_pipeline/analysis/plotting/Auto_parse_qc_heatmap.R")
}
...
sample_manifest <- read.csv(file.path("summary", "sample_manifest.csv"), stringsAsFactors = FALSE)
...
write.csv(summary_tbl, file.path("summary", "qc_heatmap_stage_summary.csv"), row.names = FALSE)
