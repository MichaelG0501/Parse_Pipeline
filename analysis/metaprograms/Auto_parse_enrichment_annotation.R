####################
# Auto_parse_enrichment_annotation.R
# Enrichment annotation for Parse metaprograms.
...
script_path <- if (length(file_arg) > 0) {
  ...
  normalizePath("/rds/general/ephemeral/project/spatialtranscriptomics/ephemeral/Parse_Pipeline/analysis/metaprograms/Auto_parse_enrichment_annotation.R")
}
...
MP_list <- read.csv("/rds/general/project/tumourheterogeneity1/live/ITH_sc/PDOs/Count_Matrix/New_NMFs.csv", check.names = FALSE)
...
if (!file.exists(mp_default_path)) {
  stop("Missing selected metaprogram object. Run Auto_parse_find_optimal_nmf.R first: ", mp_default_path)
}
