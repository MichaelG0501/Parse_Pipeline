####################
# parse_metaprogram_geneNMF_discovery.R
#
# Description:
#   Runs GeneNMF metaprogram discovery on the six Parse treatment-response
#   samples and, after optimal nMP selection, scores retained metaprograms with
#   UCell.
#
# Inputs:
#   parse_outs/by_samples/<sample>/Auto_<sample>_final.rds
#   Optional cache: parse_outs/Auto_parse_metaprograms/Auto_parse_list_geneNMF.rds
#   Optional cache: parse_outs/Auto_parse_metaprograms/Auto_parse_merged_geneNMF.rds
#
# Outputs:
...
#   parse_outs/logs/run_summaries/parse_metaprogram_geneNMF_discovery_*.txt
#
# Cache / replot:
#   Cached sample-list, merged object, GeneNMF outputs, selected MP objects,
#   final Seurat object, and UCell matrices are reused when present.
#
# Methodology:
...
script_run <- parse_start_run(
  "parse_metaprogram_geneNMF_discovery",
  parameters = list(samples = paste(parse_samples, collapse = ","), k_range = "4:35"),
...
script_path <- if (length(file_arg) > 0) {
  ...
  normalizePath(file.path(parse_project_root(), "analysis/metaprograms/parse_metaprogram_geneNMF_discovery.R"))
}
...
excluded_samples <- parse_external_samples
...
# parse_metaprogram_select_optimal_nMP.R writes Auto_parse_optimal_nMP.txt.
####################
...
if (is.na(default_nMP)) {
  message("Optimal nMP has not been selected yet. Run parse_metaprogram_select_optimal_nMP.R next.")
} else {
...
message("parse_metaprogram_geneNMF_discovery.R completed successfully.")
