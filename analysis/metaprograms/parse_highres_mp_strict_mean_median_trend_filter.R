...
#
# Description:
#   Active high-resolution Parse metaprogram trend workflow. Starts from the
#   GeneNMF program sweep, collapses to nMP117, scores all six Parse samples,
#   and retains MPs only when mean and median trends agree across the treatment
#   trajectory and recovery samples. Produces terminal figures and gene tables.
#
# Inputs:
#   parse_outs/Auto_parse_metaprograms/Auto_parse_geneNMF_outs.rds
#   parse_outs/by_samples/<sample>/Auto_<sample>_final.rds
#   Optional cache: parse_outs/Auto_parse_highres_metaprogram_trends/Auto_parse_highres_*_nMP117.*
#   3CA, cell-cycle, and developmental enrichment reference files listed in AGENTS.md
#
# Outputs:
#   parse_outs/Auto_parse_highres_metaprogram_trends/Auto_parse_highres_selected_mp_genes_nMP117.{rds,csv}
#   parse_outs/Auto_parse_highres_metaprogram_trends/Auto_parse_highres_trend_summary_nMP117.csv
#   parse_outs/Auto_parse_highres_metaprogram_trends/Auto_parse_highres_activity_boxplots_nMP117_selected.pdf
#   parse_outs/Auto_parse_highres_metaprogram_trends/Auto_parse_highres_enrichment_annotation_nMP117.pdf
#   parse_outs/Auto_parse_highres_metaprogram_trends/Auto_parse_highres_mp_genes_summary_nMP117.xlsx
#   parse_outs/logs/run_summaries/parse_highres_mp_strict_mean_median_trend_filter_*.txt
#
# Cache / replot:
#   The --mode argument supports score, enrich, excel, and all. Cached nMP117
#   metaprograms, UCell scores, cell metadata, selected genes, and trend summary
#   are reused where present, so visualization-only edits can be reproduced from
#   intermediate RDS/CSV files without rerunning GeneNMF or UCell scoring.
#
# Methodology:
...
script_path <- if (length(file_arg) > 0) {
  ...
  normalizePath(file.path(parse_project_root(), "analysis/metaprograms/parse_highres_mp_strict_mean_median_trend_filter.R"))
}

script_run <- parse_start_run(
  "parse_highres_mp_strict_mean_median_trend_filter",
  parameters = list(mode = mode, samples = paste(parse_samples, collapse = ","), score_ncores = parse_thresholds$highres_score_ncores),
  input_files = c(
    "parse_outs/Auto_parse_metaprograms/Auto_parse_geneNMF_outs.rds",
    "parse_outs/by_samples/<sample>/Auto_<sample>_final.rds",
